    ...
    switch (HC_State)
    {
        case PCK_ENUM:
        {
            ...
            else if ((Get_SysTick() - wforecast_update_timer) >= WFC_UPD_TIME)
            {
                ...
                uint32_t wf_unix = (eebuff[0]<<24)|(eebuff[1]<<16)|(eebuff[2]<<8)|eebuff[3];
                if (wf_unix && (wf_unix != 0xFFFFFFFF))
                {
                    if((dt.unix < (wf_unix + SECONDS_PER_DAY)) && (wf_unix < (dt.unix + SECONDS_PER_DAY)))
                    {
                        COM_Bridge = BR2OW;
                        rs485_txaddr = rsbra;
                        ZEROFILL(hc_buff, COUNTOF(hc_buff));
            ...
            if ((Get_SysTick() - rx_tmr) >= rx_tout)
            {
                ...
                switch (HC_Update)
                {
                    ...
                    case UPD_BINARY:
                        ...
                        break;
                    case UPD_FILE:
                        if (COM_Bridge != BR2OW) rx_tout = RESP_TOUT/2;
                        else rx_tout = BIN_TOUT;
        ...
        case PCK_RECEIVING:
        {
            ...
            {
                bcnt = 0x0U; 
                while (bcnt < (rxbcnt - 0x9U))
                {
                    if ((rx_buff[bcnt]      <= US)                              // first packet byte is ascii controll character
                    &&  (rx_buff[bcnt + 0x1U] == ((rsifa >> 8) & 0xFFU)) // second is rs485 interface address msb 
                    &&  (rx_buff[bcnt + 0x2U] == (rsifa & 0xFFU))        // third is rs485 interface address lsb
                    &&  (rx_buff[bcnt + 0x3U] == ((rs485_txaddr >> 8) & 0xFFU)) // second is receiver interface address msb 
                    &&  (rx_buff[bcnt + 0x4U] == (rs485_txaddr & 0xFFU))        // third is receiver interface address lsb
                    &&  (rx_buff[rx_buff[bcnt + 0x5U] + 0x8U] == EOT))          // last packet byte is ascii controll "end of transmission"
                    {
                        ...
                        if ((rx_buff[rx_buff[bcnt + 0x5U] + 0x6U] == ((rs485_pkt_chksum >> 8) & 0xFFU)) 
                        &&  (rx_buff[rx_buff[bcnt + 0x5U] + 0x7U] ==  (rs485_pkt_chksum & 0xFFU)))
                        {
                        ...
                        else if(rx_buff[0] == NAK)
                        {
                            ...
                            {   /* create same size variable from received packet number request */
                                ...
                                else 
                                {
                                    ...
                                    ++HC_FilUpdPck.trial;
                                    rx_tmr = Get_SysTick();
                                    rx_tout = MUTE_DEL; // try to slow down little bit
                                    HC_State = PCK_ENUM;
                                    return;
                                }
                            }
                    ...
                    case UPD_LOG:
                    {
                        if      (rx_buff[0] == ACK)  // command acknowledged
                        {
                            ...
                            else if ((rx_buff[5] == (LOG_DSIZE+2)) && (rx_buff[6] == GET_LOG_LIST))
                            {
                                ...
                                eebuff[3] = rs485_txaddr>>8; // insert sender rs485 address
                                eebuff[4] = rs485_txaddr; // for log source 
...
void HC_WriteLogEvent(uint8_t event, uint8_t type)
{
    ...
    eebuff[10] = wldt.date;
    eebuff[11] = wldt.month;
    eebuff[12] = wldt.year;
    eebuff[13] = wldt.hours;
    eebuff[14] = wldt.minutes;
    eebuff[15] = wldt.seconds;
    HC_WriteLog();
    DISP_UpdateLog();
}
/**
...
void HC_CreateLogRequest(void)
{
    ...
    if (COM_Bridge > BRNONE)
    {
        ...
        rs485_pkt_chksum = 0U;
        for (uint32_t i = 6U; i < (tx_buff[5] + 6U); i++)
        {
...
void HC_WriteLog(void)
{
    ...
    */
    ...
    {
        case TYPE_1:
            if (I2CEE_WriteBytes16(I2CEE_PAGE_0, HC_LogMemory.next_addr, eebuff, LOG_DSIZE) != 0U) NVIC_SystemReset();
            DelayMs(I2CEE_WRITE_DELAY);
            ++HC_LogMemory.log_cnt;
            HC_LogMemory.Allocation = TYPE_5;
...
void HC_CreateTimeUpdatePacket(void)
{
    ...
    */    
    ...
    tx_buff[7] = updt.weekday;
    if(updt.weekday == 0x0U) tx_buff[7] = 0x7U;
    tx_buff[8] = updt.date;
    tx_buff[9] = updt.month;
    tx_buff[10]= updt.year;
    tx_buff[11]= updt.hours;
    tx_buff[12]= updt.minutes;
    tx_buff[13]= updt.seconds;
    rs485_pkt_chksum = 0x0U;
    for (i = 0x6U; i < 14U; i++)
    {
        rs485_pkt_chksum += tx_buff[i];
...
uint8_t HC_CheckNewImageFile(void)
{
    ...
    rs485_txaddr = atoi(hc_buff);
    if ((HC_FilUpdPck.cmd == DWNLD_DISP_IMG)  && (COM_Link != GROUP))
    ...
    {    /* image format IMGxx.RAW    */
        if (COM_Link == GROUP)
        {
            ...
            Int2Str(&name[3], filupd_list[filupd_list_cnt] , 0x0U);
            i = strlen(name);
            strncat(&name[i], ".RAW", 0x4U);
            if (f_open(&file_SD, name, FA_READ) == FR_OK)                    
            {
...
void HC_CreateCmdRequest(uint8_t cmd, char *ibuf)
{
    ...
    if (COM_Bridge == BR2OW)
    {
        for (uint32_t bcnt = tx_buff[5]-1; bcnt; bcnt--)
        {
            tx_buff[bcnt+10] = tx_buff[bcnt+6]; // copy buffer to make space for header
        }        
        tx_buff[5] += 4; // add header size to packet lenght
        tx_buff[6] = SET_BR2OW; // set bridge command
        tx_buff[7] = ow_txaddr;  // onewire receiver address
        tx_buff[8] = ow_ifaddr; // onewire interface address
        tx_buff[9] = tx_buff[5]-4; // onewire data payload size
        tx_buff[10]= cmd;
    }    
    rs485_pkt_chksum = 0;
    for (i = 6; i < (tx_buff[5]+6); i++)
    {
        rs485_pkt_chksum += tx_buff[i];
    }
    tx_buff[tx_buff[5]+6] = rs485_pkt_chksum>>8;
    tx_buff[tx_buff[5]+7] = rs485_pkt_chksum;
