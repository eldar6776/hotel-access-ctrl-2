...
const char sys_info_hw[] =
{ "RedDragonF429" };   // hardware version
const char sys_info_fw[] =
{ "HC090519" };        // firmware version
const char *parstag[40] =
{ "RTsta", "RTmod", "BTdnd", "BThmd", "BTsos", "SPset", "SPmin", "SPmax", "CLmax",
        "HTmin",        // command parser
        "FNmod", "FNdif", "FNlow", "FNmid", "OWifa", "OWgra", "OWbca", "RSifa", "RSgra",
        "RSbca",        // room thermostat parameter list
        "RSbps", "RFcha", "RFrxa", "RFtxa", "SYrtc", "SYerr", "WFtxt", "LGlst", "LGdel", "QRcod", "DLfwr", "DLbld",
        "DLimg", "BZtim", "BZmod", "SYlng", "SYrst", "SYsta", "DSmsg", "DSimg" };
/* macro to construct section name string from table offset     */
/* to automatize file info placing when switching or changing   */
/* vector table offset from debugger mode to release            */
...
//const uint32_t fwsize __attribute__((section(XAFTERX(FWINFADDR)),used)) = 0x00000000U; // this firmware size
/* Imported Variable  --------------------------------------------------------*/
/* Imported Function  --------------------------------------------------------*/
/* Private Type --------------------------------------------------------------*/
/* Private Define ------------------------------------------------------------*/
...
void Int2Str(char *pstr, signed int val, uint8_t str_size)
{
	...
	for (i = 0x0U; i < 10U; i++)
	{
		...
		if (str_size == 0x0U)    //-------------------------------supress leading zeros conversion
		{
			pstr[t] = tmp[i];

			if (t == 0x0U)  //-----------------------------------check is writing enabled
			{
	...
	if (str_size != 0x0U)    //-----------------------------------if sized result requested
	{
		...
		if (s)
		{
			t = 0x1U;
			pstr[0] = '-';  //----------------------------------write minus sign
			pstr[str_size + 0x1U] = NUL;    //------------------0x00U terminate string
		}
