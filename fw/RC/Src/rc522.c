...
#define RC522_PROCESS_TIME					89U	    // 89 ms read rate
...
{	
    ...
    if (RC522_Check(serial) == MI_OK)
    {
        ...
        if (++check > 1)
        {
            ...
            mem_copy(sCard.card_id, serial, 5U);
            RC522_VerifyData();
...
//                        RC522_Init();
    ...
    else 
    {
        ...
        check = 0;
        if(!(dout_0_7_rem & (1U << 6))) DoorLockCoil_Off();
		RC522_RST_SetLow(); 
		DOUT_Service();
