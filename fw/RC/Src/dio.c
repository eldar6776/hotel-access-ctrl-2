...
uint8_t hc595_dout;
/* Private Define  -----------------------------------------------------------*/
#define INPUT_DEBOUNCE_CNT  						20      // number of input state check 10
#define FAST_INPUT_DEBOUNCE_CNT  					5       // number of input state check 5
...
#define CAP_SW_ACTIV_TIME						    876U    // 1 s handmaid capacitive switch activ time
#define MENU_BUTTON_TIME				            123U    // when cap switch used as service menu button
...
void DIO_Service(void) 
{
	...
	/** ============================================================================*/
    if      (din_cfg[0] == 5) din_state &=(~(1U<<0)), din_0_7 |=  (1U<<0);
    else if (din_cfg[0] == 4) din_state |=  (1U<<0),  din_0_7 &=(~(1U<<0));

    if      (din_cfg[1] == 5) din_state &=(~(1U<<1)), din_0_7 |=  (1U<<1);
    else if (din_cfg[1] == 4) din_state |=  (1U<<1),  din_0_7 &=(~(1U<<1));
    else if (din_cfg[1] == 2)
    {
        if      (HAL_GPIO_ReadPin(DIN_1_Port, DIN_1_Pin) == GPIO_PIN_SET)   din_state |=  (1U<<1);
        else if (HAL_GPIO_ReadPin(DIN_1_Port, DIN_1_Pin) == GPIO_PIN_RESET) din_state &=(~(1U<<1));
    }
    
    if      (din_cfg[2] == 5) din_state &=(~(1U<<2)), din_0_7 |=  (1U<<2);
    else if (din_cfg[2] == 4) din_state |=  (1U<<2),  din_0_7 &=(~(1U<<2));
    else if (din_cfg[2] == 2)
    {
        if      (HAL_GPIO_ReadPin(DIN_2_Port, DIN_2_Pin) == GPIO_PIN_SET)   din_state |=  (1U<<2);
        else if (HAL_GPIO_ReadPin(DIN_2_Port, DIN_2_Pin) == GPIO_PIN_RESET) din_state &=(~(1U<<2));
    }
    
    if      (din_cfg[3] == 5) din_state &=(~(1U<<3)), din_0_7 |=  (1U<<3);
    else if (din_cfg[3] == 4) din_state |=  (1U<<3),  din_0_7 &=(~(1U<<3));
    else if (din_cfg[3] == 2)
    {
        if      (HAL_GPIO_ReadPin(DIN_3_Port, DIN_3_Pin) == GPIO_PIN_SET)   din_state |=  (1U<<3);
        else if (HAL_GPIO_ReadPin(DIN_3_Port, DIN_3_Pin) == GPIO_PIN_RESET) din_state &=(~(1U<<3));
    }
    
    if      (din_cfg[4] == 5) din_state &=(~(1U<<4)), din_0_7 |=  (1U<<4);
    else if (din_cfg[4] == 4) din_state |=  (1U<<4),  din_0_7 &=(~(1U<<4));
    else if (din_cfg[4] == 2)
    {
        if      (HAL_GPIO_ReadPin(DIN_4_Port, DIN_4_Pin) == GPIO_PIN_SET)   din_state |=  (1U<<4);
        else if (HAL_GPIO_ReadPin(DIN_4_Port, DIN_4_Pin) == GPIO_PIN_RESET) din_state &=(~(1U<<4));
    }
    
    if      (din_cfg[5] == 5) din_state &=(~(1U<<5)), din_0_7 |=  (1U<<5);
    else if (din_cfg[5] == 4) din_state |=  (1U<<5),  din_0_7 &=(~(1U<<5));
    else if (din_cfg[5] == 2)
    {
        if      (HAL_GPIO_ReadPin(DIN_5_Port, DIN_5_Pin) == GPIO_PIN_SET)   din_state |=  (1U<<5);
        else if (HAL_GPIO_ReadPin(DIN_5_Port, DIN_5_Pin) == GPIO_PIN_RESET) din_state &=(~(1U<<5));
    }
    
    if      (din_cfg[6] == 5) din_state &=(~(1U<<6)), din_0_7 |=  (1U<<6);
    else if (din_cfg[6] == 4) din_state |=  (1U<<6),  din_0_7 &=(~(1U<<6));
    else if (din_cfg[6] == 2)
    {
        if      (HAL_GPIO_ReadPin(DIN_6_Port, DIN_6_Pin) == GPIO_PIN_SET)   din_state |=  (1U<<6);
        else if (HAL_GPIO_ReadPin(DIN_6_Port, DIN_6_Pin) == GPIO_PIN_RESET) din_state &=(~(1U<<6));
    }
    
    if      (din_cfg[7] == 5) din_state &=(~(1U<<7)), din_0_7 |=  (1U<<7);
    else if (din_cfg[7] == 4) din_state |=  (1U<<7),  din_0_7 &=(~(1U<<7));
    else if (din_cfg[7] == 2)
    {
        if      ((HAL_GPIO_ReadPin(DIN_7_Port, DIN_7_Pin) == GPIO_PIN_SET)   || (HAL_GPIO_ReadPin(DIN_0_Port, DIN_0_Pin) == GPIO_PIN_SET))   din_state |=  (1U<<7);
        else if ((HAL_GPIO_ReadPin(DIN_7_Port, DIN_7_Pin) == GPIO_PIN_RESET) && (HAL_GPIO_ReadPin(DIN_0_Port, DIN_0_Pin) == GPIO_PIN_RESET)) din_state &=(~(1U<<7));
    }
    /** ============================================================================*/                                                                         
...
{	
	...
	/** ============================================================================*/
    /*  
    *   dout_0_7 bit 0 is reversed here to be engage in room power control only to shutdown power supply when is activated
    */
    if      ( dout_cfg[0] == 0x5U) HAL_GPIO_WritePin(DOUT_0_Port, DOUT_0_Pin, GPIO_PIN_SET),   dout_0_7 |=  (1U<<0);        /* pin 30 prema Edinovoj skici          */
    else if ( dout_cfg[0] == 0x4U) HAL_GPIO_WritePin(DOUT_0_Port, DOUT_0_Pin, GPIO_PIN_RESET), dout_0_7 &=(~(1U<<0));       /* DOUT0 output direct control          */
    ...
    if      ( dout_cfg[1] == 0x5U) HAL_GPIO_WritePin(DOUT_1_Port, DOUT_1_Pin, GPIO_PIN_RESET), dout_0_7 &=(~(1U<<1));       /* DOUT1 output direct control          */
    else if ( dout_cfg[1] == 0x4U) HAL_GPIO_WritePin(DOUT_1_Port, DOUT_1_Pin, GPIO_PIN_SET),   dout_0_7 |=  (1U<<1);        /* pin 28 prema Edinovoj skici          */
    ...
    if      ( dout_cfg[4] == 0x5U) DOUT_3_SetLow(), dout_0_7 &=(~(1U<<4));          /* DOUT3 output shift register control  */
    else if ( dout_cfg[4] == 0x4U) DOUT_3_SetHigh(),dout_0_7 |=  (1U<<4);           /* DOUT3 output shift register control  */
    ...
    if      ( dout_cfg[5] == 0x5U) DOUT_4_SetLow(), dout_0_7 &=(~(1U<<5));          /* DOUT4 output shift register control  */
    else if ( dout_cfg[5] == 0x4U) DOUT_4_SetHigh(),dout_0_7 |=  (1U<<5);           /* DOUT4 output shift register control  */
	...
	if (HAL_SPI_Transmit(&hspi2, &hc595_dout, 1U, 10U) != HAL_OK) ErrorHandler(DIO_FUNC, SPI_DRV);	
...
{
#ifdef  GARAGE_CTRL
    static __IO uint32_t garage_time = 0U;
    static __IO uint32_t garage_timer = 0U;
#endif
	...
	else if ((din_state & (1U<<0)) && !IsIndorCardReaderActiv())	
	{   /*  debounce digital input 0 on state */
		if(++din_0_timer >= FAST_INPUT_DEBOUNCE_CNT)
		{        
            ...
            if (!IsPowerContactorActiv() && (dout_cfg[0] < 3))
            {
                PowerContactorOn();
	...
	{   /*  debounce digital input 0 off state */
		if(++din_0_timer >= INPUT_DEBOUNCE_CNT)
		{        
            din_0_timer = 0;        // reset bit 0 debounce counter
            din_0_7 &= (~(1U<<0));  // change input register bit 0 state
            if (IsPowerContactorActiv() && (dout_cfg[0] < 3))
	...
	else if ((din_state & (1U<<1)) && !IsSOSSwClosed())	
	{   /*  debounce digital input 1 on state */     
		if(++din_1_timer >= INPUT_DEBOUNCE_CNT)
		{        
	...
	else if ((din_state & (1U<<2)) && !IsSOSResetSwClosed())	
	...
	{   /*  debounce digital input 2 off state */ 
		if(++din_2_timer >= INPUT_DEBOUNCE_CNT)
		{
			din_2_timer = 0; 	        // reset bit 2 debounce counter
			din_0_7 &= (~(1U<<2));     // change input register bit 2 state
		}
	...
	else if ((din_state & (1U<<3)) && !IsCallMaidSwClosed())	
	...
	else if ((din_state & (1U<<4)) && !IsMinibarSwClosed())	
	{   /*  debounce digital input 4 on state */ 
		if(++din_4_timer >= INPUT_DEBOUNCE_CNT)
		{        
	...
	{   /*  debounce digital input 4 off state */ 
		...
		{        
            ...
            LEDRedOff();
#else 
            if(!IsDISP_MinibarUsedImgActiv())
            {
                DISP_MinibarUsedImage();
	...
	else if ((din_state & (1U<<5)) && !IsWinDoorSwClosed())	
	{   /*  debounce digital input 5 on state */ 
		if(++din_5_timer >= INPUT_DEBOUNCE_CNT)
		{
	...
	{   /*  debounce digital input 5 off state */ 
		...
		{        
			...
			din_0_7 &= (~(1U<<5));    // change input register bit 5 state
#ifndef  GARAGE_CTRL 
            if (IsTempRegEnabled()) TempRegDisable();
            if (IsWinDoorSwEnabled() && (din_cfg[5] < 3))
            {
	...
	else if ((din_state & (1U<<6)) && !IsDNDSwClosed())	
	...
	else if ((din_state & (1U<<7)) && !IsEntryDoorSwClosed())	
	{   /*  debounce digital input 7 on state */ 
		if(++din_7_timer >= INPUT_DEBOUNCE_CNT)
		{
	...
	{   /*  debounce digital input 7 off state */ 
		if(++din_7_timer >= FAST_INPUT_DEBOUNCE_CNT)
		{
			din_7_timer = 0; // reset bit 7 debounce counter
            din_0_7 &= (~(1U<<7));    // change input register bit 7 state
            if (!IsEntryDoorOpen() && IsEntryDoorSwEnabled())
            {
                EntryDoorOpenSet();
	...
	*/
	...
	if (IsHandmaidSwitchActiv() && !IsLeftButtonActiv())
	{
        ...
        if(IsLEDGreenActiv()) old_state = 1U;
        else old_state = 0x0U;
        StatUpdSet();
        
		if((menu.state == PENDING) || (jrnl_mod == 1U))
		{
            if(menu.state == PENDING) menu.state = PRINT;
            else if (jrnl_mod == 1U) ++jrnl_mod;
			BUZZ_State = BUZZ_DOOR_BELL;
