...
void LOGGER_Init(void)
{
    ...
    if(HAL_I2C_Master_Transmit(&hi2c1, I2CEE_ADD, log_buff, 2U, DRV_TOUT)         != HAL_OK)  ErrorHandler(LOGGER_FUNC, I2C_DRV);
    if(HAL_I2C_Master_Receive(&hi2c1, I2CEE_ADD, log_buff, LOG_DSIZE, DRV_TOUT)   != HAL_OK)  ErrorHandler(LOGGER_FUNC, I2C_DRV);
...
LOGGER_StatusTypeDef LOGGER_Read(uint8_t *buff)
{
    ...
    if (HAL_I2C_IsDeviceReady(&hi2c1, I2CEE_ADD, DRV_TRIAL, DRV_TOUT)       != HAL_OK)  ErrorHandler(LOGGER_FUNC, I2C_DRV);
    if (HAL_I2C_Master_Transmit(&hi2c1, I2CEE_ADD, log_buff, 2U, DRV_TOUT)  != HAL_OK)  ErrorHandler(LOGGER_FUNC, I2C_DRV);
    if (HAL_I2C_Master_Receive(&hi2c1, I2CEE_ADD, buff, LOG_DSIZE, DRV_TOUT)!= HAL_OK)  ErrorHandler(LOGGER_FUNC, I2C_DRV);	
    return(LOGGER_OK);
}
/**
  * @brief
