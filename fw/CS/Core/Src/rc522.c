...
static RC522_StatusTypeDef RC522_Write(uint8_t blockAddr, uint8_t *writeData);
...
{
	static uint32_t mifare_timer = 0, clr = 0;                                  // start timer value
	static uint32_t mifare_tout = 50;
...
RC522_StatusTypeDef RC522_VerifyData(void)
{
	...
	{
		--i;
		if ((sector_1.block_0[i] == USERGRP_GUEST) || (sector_1.block_0[i] == USERGRP_MAID)
		        || (sector_1.block_0[i] == USERGRP_MANAGER) || (sector_1.block_0[i] == USERGRP_SERVICER))
		{
			sCard.user_group = sector_1.block_0[i];
			break;
		}
	}
	while (i);
	sCard.system_id = Str2Int((char*) sector_1.block_1, 5);
	sCard.controller_id = (sector_2.block_0[6] << 8) | sector_2.block_0[7];
	mem_copy(sCard.expiry_time, sector_2.block_0, 6U);
	if ((sCard.user_group != USERGRP_GUEST) && (sCard.user_group != USERGRP_MAID)
	        && (sCard.user_group != USERGRP_MANAGER) && (sCard.user_group != USERGRP_SERVICER)) return (MI_ERR);
	if ((sCard.system_id == 0xFFFF) || (sCard.system_id == 0) || (sCard.system_id != DEF_SYS_ID)) return (MI_ERR);
	if ((sCard.controller_id == 0xFFFF) || (sCard.controller_id == 0)
			|| ((sCard.user_group == USERGRP_GUEST) && (sCard.controller_id != DEF_CTRL_ID))) return (MI_ERR);
			if (!ISVALIDBCD(sCard.expiry_time[0]) || !ISVALIDBCD(sCard.expiry_time[1])
					|| !ISVALIDBCD(sCard.expiry_time[2]) || !ISVALIDBCD(sCard.expiry_time[3])
					|| !ISVALIDBCD(sCard.expiry_time[4]) || !ISVALIDBCD(sCard.expiry_time[5])) return (MI_ERR);
			return (MI_OK);
		}
