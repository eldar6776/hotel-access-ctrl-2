...
void LOGGER_Init(void)
{
    ...
    if (EE_ReadData(EE_ADDR, logger_next_log_address, log_buff, LOG_DSIZE) != HAL_OK)      ErrorHandler(LOGGER_FUNC, I2C_DRV);
...
LOGGER_StatusTypeDef LOGGER_Write(void)
{
    ...
    memset(&LogEvent, 0U, sizeof(LogEvent));
    if (EE_WriteData(EE_ADDR, logger_next_log_address, log_buff, LOG_DSIZE) != HAL_OK)  ErrorHandler(LOGGER_FUNC, I2C_DRV);
    if (EE_WaitStbySta() != HAL_OK)                                                     ErrorHandler(LOGGER_FUNC, I2C_DRV);
    logger_next_log_address += LOG_DSIZE;
    ++logger_list_count;
    ++logger_next_log_id;
    if(logger_next_log_address > (EE_LOG_LIST_END_ADDR - LOG_DSIZE)) SYS_LogListFullSet();
...
LOGGER_StatusTypeDef LOGGER_Read(uint8_t *buff)
{
    ...
    if (EE_ReadData(EE_ADDR, logger_next_log_address - LOG_DSIZE, buff, LOG_DSIZE) != HAL_OK)    ErrorHandler(LOGGER_FUNC, I2C_DRV);
    return(LOGGER_OK);
}
/**
  * @brief
...
LOGGER_StatusTypeDef LOGGER_Delete(void)
{
    ...
    if (EE_WriteData(EE_ADDR, logger_next_log_address - LOG_DSIZE, log_buff, LOG_DSIZE) != HAL_OK)  ErrorHandler(LOGGER_FUNC, I2C_DRV);
    if (EE_WaitStbySta() != HAL_OK)                                                                 ErrorHandler(LOGGER_FUNC, I2C_DRV);
    logger_next_log_address -= LOG_DSIZE;
    --logger_list_count;
